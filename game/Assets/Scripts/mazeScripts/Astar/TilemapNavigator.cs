...
public class TilemapNavigator : MonoBehaviour
{
    ...
    [Tooltip("将所有可走的Tilemap都拖到这里")]
    public Tilemap[] walkableTilemaps;
    [Tooltip("将所有障碍物的Tilemap都拖到这里")]
        ...
        if (walkableTilemaps != null && walkableTilemaps.Length > 0)
        {
            foreach (var walkableTilemap in walkableTilemaps)
            {
                ...
                foreach (var pos in bounds.allPositionsWithin)
                {
                    ...
                    {
                        // 例如，如果之前被标记成 false，这里可以覆盖为true（只要识别到它是walkable Tile）
    ...
    /// <summary>
    /// 在Scene视图中绘制Gizmos，可视化哪些格子可走、哪些不可走
    /// </summary>
    private void OnDrawGizmos()
    {
        ...
        foreach (var kvp in nodeGrid)
        {
            ...
            // 如果都没有Tilemap引用，就只能用coord直接做世界坐标(仅作测试用)
            if (!foundTilemap)
            {
